  ...
  constructor(scope: Construct, id: string, props: ControlPlaneStackProps) {
    ...
    const infraProject = new codebuild.Project(this, 'ManageRunStack', {
      ...
      buildSpec: codebuild.BuildSpec.fromObject({version: '0.2', phases: {build: {commands: [
        ...
        'npx cdk "$CDK_ACTION" "OpenDdsPerformanceRun-*" --require-approval never --force -c stage="$STAGE" -c runId="$RUN_ID" -c suite="$SUITE" -c commitSha="$OPENDDS_COMMIT" -c configCommit="$CONFIG_COMMIT" -c environmentName="$ENVIRONMENT_NAME" -c instanceType="$INSTANCE_TYPE" -c amiId="$AMI_ID" -c availabilityZone="$AVAILABILITY_ZONE" -c dynamicMulticastRegistration="$DYNAMIC_MULTICAST_REGISTRATION" -c artifactKey="builds/$BUNDLE_VERSION/$OPENDDS_COMMIT/bench.tar.gz" -c configKey="configs/$CONFIG_COMMIT/config.tar.gz"',
      ]}}}),
